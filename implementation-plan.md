...
**Status:** Slices 0 → 5 complete; slice 6A (Bitbucket adapter, single-VCS swap) complete; slices 6B, 7, 8 planned
...
| 6B. Multi-VCS routing (deploy GH + Bitbucket together) | Not started | Introduce a `VcsRegistry` port keyed by provider; add a `repos.provider` column with migration that defaults existing rows to `github`; split `[vcs]` into `[vcs.github]` and `[vcs.bitbucket]` config blocks; route at the webhook gateway based on the path (`/github/webhook` vs `/bitbucket/webhook`); pipelines resolve the right `VcsSource` per `PrRef.Provider`. |
...
| 8. Conversation mode (bot replies to questions) | Not started | When a human reply on a bot inline comment ends with `?` or `/explain`, treat it as a clarification request. New pipeline `cmd/conversation-worker` (or a sub-handler in feedback-worker) consumes the `pull_request_review_comment` event, runs a focused LLM call with `[ORIGINAL COMMENT] + [USER REPLY] + clarify`, and posts a nested reply. Hard cap of 2-3 bot replies per PR (configurable) to prevent LLM loops; charged against the same daily USD cap. Skipped by design today: replies record `outcome=discussed/signal=replied` for retrieval-weighting purposes, but the bot never speaks again. |
